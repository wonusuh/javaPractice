...
public class Controller {
	...
	public Controller() {
		...
		String stuData = Utils.getData("student.txt");
		String subData = Utils.getData("subject.txt");
	...
	public void run() {
		while (true) {
			...
			if (sel == 0) { // 종료
				...
				Utils.saveStudents("student.txt", stuDAO.stuList);
				Utils.saveSubjects("subject.txt", subDAO.subList);
			} else if (sel == 8) { // 파일로드
				String stuData = Utils.getData("student.txt");
				String subData = Utils.getData("subject.txt");
				stuDAO.putDataIn(stuData);
