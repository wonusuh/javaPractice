...
public class Utils {
	...
	public static String getData(String fileName) { // 파일을 읽어서 String에 저장하고 리턴합니다.
	...
	public static void saveStudents(String fileName, ArrayList<Student> stuList) { // 배열을 받아와서 파일에 저장합니다.
		File f = new File(CUR_PATH + fileName);
		if (!f.exists()) {
	...
	public static void saveSubjects(String fileName, ArrayList<Subject> subList) { // 배열을 받아와서 파일에 저장합니다.
		File f = new File(CUR_PATH + fileName);
		if (!f.exists()) {
			System.out.println(fileName + " 이 존재하지 않습니다");
			return;
