...
//	5. 도착하면 다시 -10 ~ 10 사이의 랜덤 목적지가 설정된다
		...
		while (true) {
			...
			System.out.printf("[방향] == %d, [속력] == %d\n", dir, speed);
			System.out.printf("[요금] == %d원, [전체수입] == %d원\n", fee, earnings);
			System.out.println("[1. 속도][2. 방향][3. 이동][0. 종료]");
			int menu = sc.nextInt();
			if (menu == 1) {
				while (true) {
					System.out.println("설정할 속력을 입력하세요 (1 ~ 3)>>");
					speed = sc.nextInt();
					if (speed < 1 || speed > 3) {
						System.out.println("속력은 1 ~ 3 으로만 설정할 수 있습니다.");
						continue;
					}
					break;
				}
				...
				while (true) {
					...
					dir = sc.nextInt();
					if (dir < 1 || dir > 4) {
						System.out.println("방향설정 오류 : (1~4)");
						continue;
					}
					break;
				}
				...
				if (dir == 0 || speed == 0) {
					System.out.println("먼저 방향과 속력을 설정하세요");
					continue;
				} else if (dir == 1) {
					x += speed;
				} else if (dir == 2) {
				...
				fee += speed * 50;
				if (x == xd && y == yd) {
					System.out.println("목적지에 도착했습니다. 요금 : " + fee);
					earnings += fee;
					fee = 0;
				...
				System.out.println("종료합니다. 전체수입 == " + earnings + "원");
				break;
			} else {
				System.out.println("메뉴를 다시 입력하세요(0~3) >>");
			}
		}
		sc.close();
	}
